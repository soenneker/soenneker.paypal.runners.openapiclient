...
namespace Soenneker.PayPal.Runners.OpenApiClient.Utils;
...
public sealed class FileOperationsUtil : IFileOperationsUtil
{
    ...
    public async ValueTask Process(CancellationToken cancellationToken = default)
    {
        ...
        string openApiSourceUrl = _configuration["PayPal:ClientGenerationUrl"] ?? "https://github.com/paypal/paypal-rest-api-specifications/tree/main/openapi";

        (string openApiGitUrl, string? repositorySubdirectory) = ResolveGitRepositorySource(openApiSourceUrl);

        OpenApiDocument merged = await _openApiMerger.MergeGitUrl(openApiGitUrl, repositorySubdirectory, cancellationToken);
        ...
        await _processUtil.Start("kiota", gitDirectory, $"kiota generate -l CSharp -d \"{fixedFilePath}\" -o src/{Constants.Library} -c PayPalOpenApiClient -n {Constants.Library}",
